...
namespace UnityEngine.Experimental.Rendering.HDPipeline
{
    public class HDRenderPipeline : RenderPipeline
    {
        ...
        // A set of textures that are MSAA-Only given that the full pipeline is not MSAA
        RTHandleSystem.RTHandle m_AmbientOcclusionMSAABuffer;
        ...
        void InitializeRenderTextures()
        {
            ...
            m_SharedRTManager.InitSharedBuffers(m_GbufferManager, m_Asset.renderPipelineSettings, m_Asset.renderPipelineResources, NeedDepthBufferCopy());
            ...
            m_CameraColorBuffer = RTHandles.Alloc(Vector2.one, filterMode: FilterMode.Point, colorFormat: RenderTextureFormat.ARGBHalf, sRGB: false, enableRandomWrite: true, name: "CameraColor");
            ...
            if (m_Asset.renderPipelineSettings.supportMSAA)
            {
                ...
                if (m_Asset.renderPipelineSettings.supportSSAO)
                {
                    m_AmbientOcclusionMSAABuffer = RTHandles.Alloc(Vector2.one, filterMode: FilterMode.Bilinear, colorFormat: RenderTextureFormat.RG16, sRGB: false, enableRandomWrite: true, name: "AmbientOcclusionMSAA");
                }
        ...
        void DestroyRenderTextures()
        {
            ...
            RTHandles.Release(m_AmbientOcclusionMSAABuffer);
        ...
        void CopyDepthBufferIfNeeded(CommandBuffer cmd)
        {
            using (new ProfilingSample(cmd, NeedDepthBufferCopy() ? "Copy DepthBuffer" : "Set DepthBuffer", CustomSamplerId.CopySetDepthBuffer.GetSampler()))
            {
                if (NeedDepthBufferCopy())
                {
                    ...
                    {
                        cmd.CopyTexture(m_SharedRTManager.GetDepthStencilBuffer(), m_SharedRTManager.GetDepthTexture());
                    }
                }
            }
        }
        ...
        public override void Render(ScriptableRenderContext renderContext, Camera[] cameras)
        {
            ...
            foreach (var camera in cameras)
            {
                ...
                using (new ProfilingSample(cmd, "HDRenderPipeline::Render", CustomSamplerId.HDRenderPipelineRender.GetSampler()))
                {
                    ...
                    m_SharedRTManager.BindNormalBuffer(cmd, false);
                    ...
                    else
                    {
                        ...
                        using (new ProfilingSample(cmd, "Screen space shadows", CustomSamplerId.ScreenSpaceShadows.GetSampler()))
                        {
                            ...
                            m_LightLoop.RenderScreenSpaceShadows(hdCamera, m_ScreenSpaceShadowsBuffer, m_SharedRTManager.GetDepthTexture(), cmd, currentFrameSettings.enableMSAA ? m_SharedRTManager.GetDepthValuesTexture() : null);
                        ...
                        if (currentFrameSettings.enableMSAA)
                        {
                            m_SSSBufferManager.SubsurfaceScatteringPass(hdCamera, cmd, diffusionProfileSettings,
                            m_CameraColorBuffer, m_CameraSssDiffuseLightingBuffer, m_SharedRTManager.GetDepthStencilBuffer(), m_SharedRTManager.GetDepthTexture(), m_CameraColorMSAABuffer, m_SharedRTManager.GetDepthStencilBuffer(true));
                        }
                        else
                        {
                            m_SSSBufferManager.SubsurfaceScatteringPass(hdCamera, cmd, diffusionProfileSettings,
                            m_CameraColorBuffer, m_CameraSssDiffuseLightingBuffer, m_SharedRTManager.GetDepthStencilBuffer(), m_SharedRTManager.GetDepthTexture(), null, null);
                        }
            ...
            if (hdCamera.frameSettings.enableSSAO && postProcessLayer != null && postProcessLayer.enabled)
            {
                ...
                if (settings.IsEnabledAndSupported(null))
                {
                    if(frameSettings.enableMSAA)
                    {
                        ...
                        postProcessLayer.BakeMSVOMap(cmd, camera, m_AmbientOcclusionMSAABuffer, m_SharedRTManager.GetDepthValuesTexture(), true, true);
                    }
                    else
                    {
                    ...
                    if (frameSettings.enableMSAA)
                    {
                        using (new ProfilingSample(cmd, "Resolve AO Buffer", CustomSamplerId.BlitDebugViewMaterialDebug.GetSampler()))
                        {
                            ...
                            block.SetTexture("_DepthValuesTexture", m_SharedRTManager.GetDepthValuesTexture());
                            block.SetTexture("_MultiAOTexture", m_AmbientOcclusionMSAABuffer);
                            Material aoResolve = CoreUtils.CreateEngineMaterial(m_Asset.renderPipelineResources.aoResolve);
                            cmd.DrawProcedural(Matrix4x4.identity, aoResolve, 0, MeshTopology.Triangles, 3, 1, block);
                        }
                    }
                    return;
                }
        ...
        void RenderSky(HDCamera hdCamera, CommandBuffer cmd, FrameSettings frameSettings)
        {
            ...
            if(frameSettings.enableMSAA)
            {
                m_SkyManager.RenderSky(hdCamera, m_LightLoop.GetCurrentSunLight(), m_CameraColorMSAABuffer, m_SharedRTManager.GetDepthStencilBuffer(true), m_CurrentDebugDisplaySettings, cmd);
            }
            else
            {
                m_SkyManager.RenderSky(hdCamera, m_LightLoop.GetCurrentSunLight(), m_CameraColorBuffer, m_SharedRTManager.GetDepthStencilBuffer(), m_CurrentDebugDisplaySettings, cmd);
            }
                ...
                if (pass == ForwardPass.Opaque)
                {
                    ...
                    if (hdCamera.frameSettings.enableSubsurfaceScattering)
                    {
                        ...
                        if(frameSettings.enableMSAA)
                        {
                            ...
                            HDUtils.SetRenderTarget(cmd, hdCamera, m_MRTWithSSS, m_SharedRTManager.GetDepthStencilBuffer(true));
                        }
                        else
                        {
                            ...
                            HDUtils.SetRenderTarget(cmd, hdCamera, m_MRTWithSSS, m_SharedRTManager.GetDepthStencilBuffer());
                        }
                    }
                    else
                    {
                        if(frameSettings.enableMSAA)
                        {
                            HDUtils.SetRenderTarget(cmd, hdCamera, m_CameraColorMSAABuffer, m_SharedRTManager.GetDepthStencilBuffer(true));
                        }
                        else
                        {
        ...
        void RenderDepthPyramid(HDCamera hdCamera, CommandBuffer cmd, ScriptableRenderContext renderContext, FullScreenDebugMode debugMode)
        {
            ...
            using (new ProfilingSample(cmd, "Depth Pyramid", CustomSamplerId.DepthPyramid.GetSampler()))
                m_BufferPyramid.RenderDepthPyramid(hdCamera, cmd, renderContext, m_SharedRTManager.GetDepthTexture(), cameraRT);
            ...
            using (new ProfilingSample(cmd, "ClearBuffers", CustomSamplerId.ClearBuffers.GetSampler()))
            {
                ...
                using (new ProfilingSample(cmd, "Clear Depth/Stencil", CustomSamplerId.ClearDepthStencil.GetSampler()))
                {
                    if (hdCamera.clearDepth)
                    {
                        ...
                        if (frameSettings.enableMSAA)
                        {
                            HDUtils.SetRenderTarget(cmd, hdCamera, m_SharedRTManager.GetDepthAsColorBufferMSAA(), m_SharedRTManager.GetDepthStencilBuffer(true), ClearFlag.Color, Color.black);
                        }
                    }
                }
